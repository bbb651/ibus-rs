...
mod sealed {
    pub trait Sealed {}
    impl<T: super::IsA<super::Config>> Sealed for T {}
}

pub trait ConfigExt: IsA<Config> + sealed::Sealed + 'static {
    ...
    fn connect_value_changed<F: Fn(&Self, &str, &str, &glib::Variant) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"value-changed\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(value_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
