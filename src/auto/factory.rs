...
mod sealed {
    pub trait Sealed {}
    impl<T: super::IsA<super::Factory>> Sealed for T {}
}

pub trait FactoryExt: IsA<Factory> + sealed::Sealed + 'static {
    ...
    fn connect_create_engine<F: Fn(&Self, &str) -> Option<Engine> + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"create-engine\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(create_engine_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
