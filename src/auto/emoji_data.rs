...
impl EmojiData {
    ...
    #[doc(alias = "ibus_emoji_data_save")]
    pub fn save(path: &str, list: &[EmojiData]) {
        assert_initialized_main_thread!();
        unsafe {
            ffi::ibus_emoji_data_save(path.to_glib_none().0, list.to_glib_none().0);
        }
    }
}
...
mod sealed {
    pub trait Sealed {}
    impl<T: super::IsA<super::EmojiData>> Sealed for T {}
}

pub trait EmojiDataExt: IsA<EmojiData> + sealed::Sealed + 'static {
    ...
    #[doc(alias = "ibus_emoji_data_set_annotations")]
    fn set_annotations(&self, annotations: Vec<glib::GString>) {
        unsafe {
            ffi::ibus_emoji_data_set_annotations(self.as_ref().to_glib_none().0, annotations.into_glib_ptr());
        }
    }
    ...
    fn connect_annotations_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"notify::annotations\0".as_ptr() as *const _,
                Some(std::mem::transmute::<_, unsafe extern "C" fn()>(notify_annotations_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
